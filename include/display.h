/*
 * Defines/structures/prototypes related to rendering data on the screen
 *
...
# define BYTE_DIGITS                                                          \
  ( display_info.cursor_window == WINDOW_HEX ? HEX_BYTE_DIGITS                \
                                            : ASCII_BYTE_DIGITS )
# define SPACE_DIGITS 1

/* each hex byte is represented in hex by 2(hex)/8(binary) digits, plus each
 * grouping has one trailing space */
# define HEX_BYTES_PER_GROUP                                                  \
  (( user_prefs[GROUPING].value * HEX_BYTE_DIGITS ) + SPACE_DIGITS )
# define ASCII_BYTES_PER_GROUP ( user_prefs[GROUPING].value * ASCII_BYTE_DIGITS )
# define BYTES_PER_GROUP                                                      \
  ( display_info.cursor_window == WINDOW_HEX ? HEX_BYTES_PER_GROUP            \
                                            : ASCII_BYTES_PER_GROUP )
...
# define HEX_COLS                                                             \
  ( user_prefs[MAX_COLS].value == 0             ? MAX_HEX_COLS                \
   : MAX_HEX_COLS > user_prefs[MAX_COLS].value ? user_prefs[MAX_COLS].value   \
                                               : MAX_HEX_COLS )
...
# define HEX_BOX_W                                                            \
  ( HEX_BOX_W_ > ( HEX_BYTES_PER_GROUP + 2 ) ? HEX_BOX_W_                     \
                                          : ( HEX_BYTES_PER_GROUP + 2 ))
...
# define ASCII_BOX_W                                                          \
  ( ASCII_BOX_W_ > ( user_prefs[GROUPING].value + 2 )                         \
       ? ASCII_BOX_W_                                                         \
       : ( user_prefs[GROUPING].value + 2 ))
...
# define PAGE_END                                                             \
  ( _PAGE_END > display_info.file_size                                        \
...
# define MAX_MSG_BOX_LEN (( MSG_BOX_H - 3 ) * ( MSG_BOX_W - 2 ) + 1 )
...
void pat_err(const char *error, const char *pattern, int index,
             int max_index);
...
int print_line(off_t page_addr, off_t line_addr, char *screen_buf,
...
void print_screen_buf(off_t addr, char *screen_buf, size_t screen_buf_size,
