...
void
search_state_reset(search_state_t *search_state)
{
  int i;

  search_state->criteria_index = 0;
  for (i = 0; i < search_item[current_search].compiled_pattern.criteria_count;
       i++)
    {
...
int
matches(unsigned char byte, match_criteria_t *criteria)
{
  int i;
...
search_result_t
rollback(unsigned char byte, search_state_t *search_state)
{
  int i;
...
search_result_t
feed_search(char byte, search_state_t *search_state)
{
  int i;
  compiled_pattern_t *cpat = &search_item[current_search].compiled_pattern;
                  ...
                  if (search_state->match_state[i] == UNFULFILLED)
                    {
                      ...
                      if (i == search_state->criteria_index)
                        { /* check also next criteria; this can be an empty set */
              ...
              switch (cpat->criteria[i]->wildcard)
                ...
                case NONE_OR_ONE:
                  if (search_state->match_state[i] == UNFULFILLED
                    ...
                    { /* check also next criteria; can be an empty set */
                      search_state->criteria_index++;
                    }
...
void
buf_search(search_aid_t *search_aid)
{
  ...
  do
    {
      ...
      do
        {
          result = feed_search(
            search_aid->buf[start_offset + end_offset],
            &search_state);
...
void
free_compiled_pattern(compiled_pattern_t *cpat)
{
  int i;
          ...
          switch (pattern[i])
            ...
            case '?':
              ...
              if (c == NULL)
                {
                  pat_err(
                    "? must proceed a valid set or char",
                    pattern,
                    i,
                    MAX_SEARCH_PAT_LEN);
                  search_item[current_search].used = FALSE;
            ...
            case ']':
              if (buildset == 0)
                {
                  pat_err("No active set", pattern, i, MAX_SEARCH_PAT_LEN);
                  search_item[current_search].used = FALSE;
                  return;
                }
              ...
              if (buildrange == 1)
                {
                  pat_err("Invalid char for range", pattern, i, MAX_SEARCH_PAT_LEN);
                  search_item[current_search].used = FALSE;
                  return;
                }
              ...
              if (c->range_count == 0)
                {
                  pat_err("Empty range invalid", pattern, i, MAX_SEARCH_PAT_LEN);
                  search_item[current_search].used = FALSE;
                  return;
                }
            ...
            case '-':
              if (buildset == 0)
                {
                  pat_err(
                    "Can only build range within a set",
                    pattern,
                    i,
                    MAX_SEARCH_PAT_LEN);
                  search_item[current_search].used = FALSE;
              ...
              if (buildrange == 1)
                {
                  pat_err("Already building range", pattern, i, MAX_SEARCH_PAT_LEN);
                  search_item[current_search].used = FALSE;
                  return;
                }
              ...
              if (c == NULL)
                {
                  pat_err(
                    "- must proceed a valid character",
                    pattern,
                    i,
                    MAX_SEARCH_PAT_LEN);
                  search_item[current_search].used = FALSE;
      ...
      if (search_item[current_search].search_window == SEARCH_HEX)
        {
          if (( i + 1 ) >= len)
            {
              pat_err(
                "Hex chars should have two nibbles",
                pattern,
                i,
                MAX_SEARCH_PAT_LEN);
              search_item[current_search].used = FALSE;
          ...
          if (!is_hex(pattern[i]))
            {
              pat_err("Invalid hex digit", pattern, i, MAX_SEARCH_PAT_LEN);
              search_item[current_search].used = FALSE;
              return;
            }
          ...
          if (!is_hex(pattern[i + 1]))
            {
              pat_err("Invalid hex digit", pattern, i + 1, MAX_SEARCH_PAT_LEN);
              search_item[current_search].used = FALSE;
              return;
            }
      ...
      if (buildrange)
        {
          if (c->range_count < 1)
            {
              pat_err("Invalid range", pattern, i, MAX_SEARCH_PAT_LEN);
              search_item[current_search].used = FALSE;
              return;
            }
...
void
fill_search_buf(off_t addr, int display_size, search_aid_t *search_aid,
                search_direction_t direction)
{
  ...
  while (search_aid->hl_start
         != -1) /* while we continue to have valid matches */
    {
      if (direction == SEARCH_FORWARD)
        { /* find first match which has an end point past the display addr */
        ...
        { /* find last match which has a start point before the display addr */
          if (search_aid->hl_start < ( addr + display_size ))
            {
              tmp_aid = *search_aid;
            }
          else
      ...
      buf_search(
        search_aid); /* if the condition is not met find the next match */
    }
...
void
free_search_buf(search_aid_t *search_aid)
{
  if (search_aid == NULL)
    {
      return;
