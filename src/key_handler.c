...
#define WHITESPACE(x)   \
  ( x == ' ' || x == '\t' || !isprint(x))  /* includes all non-print chars */
  ...
  for (i = 0; i < num_elements; i++)
    {
      ...
      if (user_prefs[i].flags == P_INT)
        {
          snprintf(text[i + 1] + len, 256 - len, "= %'d", user_prefs[i].value);
        }
...
static int
all(const struct dirent *unused)
{
  return 1;
}
BOOL
file_browser(const char *dir, char *fname, int name_len)
{
      ...
      if (update)
        {
          ...
          mvwprintw(
            ...
            SCROLL_BOX_H - 2,
            1,
            " [j|DOWN] Down  [k|UP] Up  [ENTER|g] Select  [q|ESC] Cancel |");
          wrefresh(fb);
        }
...
action_code_t
do_search(int c, cursor_t cursor)
{
...
action_code_t
word_move(int c, cursor_t cursor)
{
  int i, size = 0;
  ...
  /*
   * This seems like an ugly hack to me, but when a user presses 'w' they
   * expect to go to the next word, but when they type 'cw' they expect to
   * change the current word, and not the trailing whitespace or the first
  ...
  switch (c)
    {
    case 'W': /* next word, only whitespace are delimiters */
      require_whitespace = 1;

    /* no break */
    case 'w': /* next word, any non-alphanumeric is delimiter,
               * and whitespace delimits groups of delimiters */
      while (size > 1)
        {
          for (i = 0; i < size; i++)
            {
...
action_code_t
word_move_back(int c, cursor_t cursor)
{
  int i, size = 0;
...
do_insert(int count, int c)
{
  ...
  int hy, hx, ay, ax;
  int ins_buf_size;
  int chars_per_byte, char_count = 0, tmp_char_count = 0,
    low_tmp_char_count = 0;
  int offset = 0, len1, ins_buf_offset, len2, len3, screen_len;
  off_t ins_addr, page_start;

  screen_buf = (char *)malloc(
    2 * PAGE_SIZE);   /* fix this later, but make it big for now */
  ...
  while (c2 != ESC)
    {
      ...
      if (len3 > 0)
        {
          screen_len += vf_get_buf(
            current_file,
            screen_buf + len1 + len2 + user_prefs[GROUPING].value,
            ins_addr,
            len3);
        }
      ...
      for (i = 0; i < user_prefs[GROUPING].value;
           i++) /* print from temp buf here to clear or print partial insert */
        {
          if (i >= tmp_char_count)
            {
          ...
          if (count > 0)
            {
              ...
              for (i = 0; i < count / user_prefs[GROUPING].value; i++)
                {
                  memcpy(
                    tmp_fill + i * ( user_prefs[GROUPING].value ),
                    replace_buf,
...
void
do_change(int multiplier, int c)
{
  off_t end_addr = 0;

  /*
   * Later we may want to create a real change operation on the file
   * back-end, but for now we have to do a delete/insert, so just do
   * it as two operations, which will make this implementation easy
   */
...
handle_key(int c)
{
  ...
  switch (c)
    ...
    case '@': /* macro playback */
    { /* don't use mgetch here; we don't want this key if recording a macro */
      int i, k = getch();
      ...
      if (macro_key != -1)
        {
           ...
           * Remove the @ from the key list and allow all the keys from one macro
           * to go into the new one In the future we could use mungetch() which
           * would increment an 'ignore' count, causing mgetch or mwgetch to ignore
           * the keys pushed on to the stack by this macro. Then we could record
           * the @<key> directly into this macro
           */
    ...
    case 'q': /* macro record */
      if (macro_key == -1)
        ...
        {
          macro_record[macro_key]
          .key_index--; /* don't save the macro-closing 'q' key */
          macro_key = -1;
        }
