...
static void insert_new_vbuf(vbuf_t **new, vbuf_t *current, vbuf_t *parent,
                            off_t offset, size_t len, buf_type_e buf_type,
                            char *src_buf);

static off_t
get_internal_shift(vbuf_t *vb)
{
  off_t shift = 0;
...
void
mod_start_offset(vbuf_t *vb, off_t shift, BOOL increase)
{
  vbuf_t *tmp = vb;
...
void
mod_parent_size(vbuf_t *vb, off_t shift, BOOL increase)
{
  vbuf_t *tmp = vb;
...
void
prune(vbuf_undo_list_t *undo_list)
{
...
static void
_cleanup_undo_vb(vbuf_list_t *vb_list)
{
...
static void
_cleanup_undo(vbuf_undo_list_t *undo_list)
{
  if (NULL == undo_list)
    {
      return;
...
void
compute_percent_complete(off_t offset, off_t size, int *complete)
{
  if (size == 0)
    {
      return;
...
/* XXX make this not void, and handle mem alloc errors? */
static void
insert_new_vbuf(vbuf_t **new, vbuf_t *current, vbuf_t *parent, off_t offset,
...
size_t
_insert_before(vbuf_t *vb, char *buf, off_t offset, size_t len,
               vbuf_undo_list_t **undo_list)
{
  vbuf_t *tmp = NULL, *new = NULL;
...
size_t
_replace(vbuf_t *vb, char *buf, off_t offset, size_t len,
         vbuf_list_t **vb_list)
{
  vbuf_t *tmp = NULL, *new = NULL;
  vbuf_list_t *tmp_vb_list = NULL;
  off_t tmp_offset = 0;
  size_t tmp_len = 0, result = 0;
...
*  ---------------------------*/
size_t
_delete(vbuf_t *vb, off_t offset, size_t len, vbuf_list_t **vb_list)
{
  vbuf_t *tmp = NULL, *new = NULL;
  vbuf_list_t *tmp_vb_list = NULL;
  off_t tmp_offset = 0;
...
/*
 * Must keep a running offset cause but[offset] is
 * not right if there are inserts/dels
 */
char
_get_char(vbuf_t *vb, char *result, off_t offset)
{
  off_t shift = 0;
  vbuf_t *tmp = vb->first_child;
  char value;
  ...
  /*
   * It is definitely in this set and we didn't
   * find it in the sub vbs, so it must be this one
   */
...
size_t
_get_buf(vbuf_t *vb, char *dest, off_t offset, size_t len)
{
  vbuf_t *tmp = NULL;
      ...
      if (tmp_offset < tmp->start)
        {
          ...
          switch (vb->buf_type)
            ...
            case TYPE_REPLACE:
              memcpy(
                dest + len - tmp_len,
                vb->buf + tmp_offset + shift - vb->start,
                read_len);
              result = read_len;
              break;
