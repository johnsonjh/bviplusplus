/*
 * Implementation for most actions that occur as a result of user input
 *
...
run_external(void)
{
  ...
  if (!is_visual_on())
    {
      msg_box(
        "Please first use visual select ('v') to select the bytes to "
        "send to the external program");
      signal(SIGPIPE, s);
      return;
    }
  else
  ...
  if (pid)
    {
      ...
      if (buf == NULL)
        {
          msg_box(
            "Error allocating buffer for external program output (size = %'d)",
            size);
          free(buf);
          close(inpipe[0]);
          close(outpipe[1]);
          ...
          if (i >= size)
            {
              ...
              if (tmp_buf == NULL)
                {
                  msg_box(
                    "Error allocating buffer for external program output (size "
                    "= %'d)",
                    size);
                  free(buf);
                  close(inpipe[0]);
  ...
  else
    {
      ...
      error = errno;
      while (read(outpipe[0], &dummy, 1) > 0
             && dummy != EOF) /* prevent SIGPIPE to parrent */
        {
...
/** UP **/
action_code_t
action_cursor_move_up(int count, cursor_t cursor)
{
...
/** DOWN **/
action_code_t
action_cursor_move_down(int count, cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
  off_t a;
...
/** LEFT **/
action_code_t
action_cursor_move_left(int count, cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
  off_t a;
...
/** RIGHT **/
action_code_t
action_cursor_move_right(int count, cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
  off_t a;
...
action_code_t
action_cursor_move_line_start(cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
  int x, y;
  off_t a;
...
action_code_t
action_cursor_move_line_end(cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
  int y;
  off_t a;
...
action_code_t
action_cursor_move_file_start(cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
...
action_code_t
action_cursor_move_file_end(cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
...
action_code_t
action_cursor_move_page_down(cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
  off_t a;
...
action_code_t
action_cursor_move_page_up(cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
  off_t a;
...
action_code_t
action_jump_to(off_t jump_addr, cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
...
action_code_t
action_delete(int count, off_t end_addr)
{
  action_code_t error = E_SUCCESS;
  off_t addr;
  int range;
...
action_code_t
action_insert_before(int count, char *buf, int len)
{
  action_code_t error = E_SUCCESS;
...
action_code_t
action_insert_after(int count, char *buf, int len)
{
  action_code_t error = E_SUCCESS;
  off_t ins_addr;
...
action_code_t
action_set_yank_register(int c)
{
  action_code_t error = E_SUCCESS;
...
action_paste_before(int count)
{
  ...
  update_display_info();
  place_cursor(
    display_info.cursor_addr + yank_buf[yank_register].len - 1,
    CALIGN_NONE,
    CURSOR_REAL);
  print_screen(display_info.page_start);
...
action_paste_after(int count)
{
  ...
  update_display_info();
  place_cursor(
    display_info.cursor_addr + yank_buf[yank_register].len,
    CALIGN_NONE,
    CURSOR_REAL);
  print_screen(display_info.page_start);
...
action_code_t
action_yank(int count, off_t end_addr, BOOL move_cursor)
{
  action_code_t error = E_SUCCESS;
  off_t addr;
  int range;
  ...
  if (address_invalid(addr) == 0)
    {
      ...
      if (bigcount > BIG_YANK)
        {
          proceed = msg_prompt(
            "Big yank may take a very long time, are you sure? (If deleting, "
            "delete will still succeed and undo will still work).",
            bigcount);
        }
...
action_code_t
action_replace(int count, char *buf, int buf_size)
{
  action_code_t error = E_SUCCESS;
...
action_code_t
action_cursor_toggle_hex_ascii(void)
{
  action_code_t error = E_SUCCESS;
...
int
action_visual_select_check(void)
{
  return is_visual_on();
}
action_code_t
action_visual_select_on(void)
{
  action_code_t error = E_SUCCESS;
...
action_code_t
action_visual_select_toggle(void)
{
  action_code_t error = E_SUCCESS;
...
void *
search_status_update_thread(void *search_data)
{
  int i = 0, c = 0;
  ...
  while (search_data_p->current != search_data_p->end && c != ESC)
    {
      if (search_data_p->start >= search_data_p->end) /* reverse search */
        {
          complete = (( search_data_p->start - search_data_p->current ) * 100 )
                     / ( search_data_p->start - search_data_p->end );
        }
      else /* forward search */
        {
          complete = (( search_data_p->current - search_data_p->start ) * 100 )
                     / ( search_data_p->end - search_data_p->start );
        }
...
action_code_t
action_move_cursor_prev_search(cursor_t cursor)
{
  action_code_t error = E_SUCCESS;
  ...
  pthread_create(
    &search_status_thread,
    &attr,
    search_status_update_thread,
    (void *)&search_data);
  ...
  if (search_data.abort == 1)
    ...
    {
      msg_box("Search term \"%s\" not found", search_item[current_search].pattern);
    }
...
action_code_t
action_move_cursor_next_search(cursor_t cursor, BOOL advance_if_current_match)
{
  action_code_t error = E_SUCCESS;
  search_aid_t search_aid;
  off_t addr;
  ...
  /* first search to make sure the overlap doesn't cause backward jump */
  while (search_aid.hl_start != -1
         && search_aid.hl_start <= display_info.cursor_addr)
    {
      if (!advance_if_current_match
          && search_aid.hl_start == display_info.cursor_addr)
        {
          break;
        }
...
action_code_t
action_do_search(int s, char *cmd, cursor_t cursor,
...
action_code_t
action_clear_search_highlight(void)
{
  action_code_t error = E_SUCCESS;
...
BOOL
file_name_prompt(char *file_name)
{
  WINDOW *w;
  int y;
  char *fname;
...
void *
save_status_update_thread(void *percent_complete)
{
      ...
      if (size != display_info.file_size)
        {
          complete = 100;
          msg_box(
            "Only saved %'jd bytes, should have saved %'jd bytes!!",
            size,
...
static action_code_t
action_blob_shift(int count)
{
...
action_code_t
action_blob_shift_right(int count)
{
  return action_blob_shift(count == 0 ? 1 : count);
}

action_code_t
action_blob_shift_left(int count)
{
  return action_blob_shift(count == 0 ? -1 : -count);
}

action_code_t
action_blob_inc(void)
{
  int new_blob_value;
