/*
 * User preferences and related functions
 *
...
action_code_t
set_pref(const char *option, const char *value)
{
          ...
          if (user_prefs[opt].flags == P_INT)
            {
              if (value == NULL)
                {
                  msg_box(
                    "Not enough parameters to 'set %s', using default value: %'d",
                    user_prefs[opt].name,
                    user_prefs[opt].def);
              ...
              if (( val < user_prefs[opt].min && user_prefs[opt].min )
                ...
                {
                  msg_box(
                    "Value out of range for 'set %s' (min = %'d, max = %'d)",
                    user_prefs[opt].name,
                    user_prefs[opt].min,
                    user_prefs[opt].max);
                  return E_INVALID;
...
read_rc_file(void)
{
  ...
  snprintf(rcfile, FILENAME_MAX, "%s/.bviplusplusrc", home);

  fp = fopen(rcfile, "r");
  if (fp == NULL)
    {
      printf("No .bviplusplusrc file found at %s\n", rcfile);
      return;
    }
          ...
          if (option != NULL && option[0] != '#')
            {
              ...
              if (error != E_SUCCESS)
                {
                  printf(
                    "Error parsing commands near character %'ld in file %s\n",
                    ftell(fp),
                    rcfile);
                }
            }
